...
**Target version: v1.4.2** (update to Implementation Roadmap v1.4.1). **Correction pass (this revision):** two fixes applied, matching the corrections made in the source v5 synthesis and the Proposals/WIP update. (1) The "Confirmed procedural rules" list previously included lazy evaluation alongside three genuinely replicated rules; split so implementation doesn't carry that rule with the same confidence as the other three. (2) The anatomical-mapping bullet now cross-references the structural-destruction case explicitly, matching the same caveat added to v5 §4/§6 and Proposals §2.1A/§2.5. No status, dependency, or acceptance-test content changed beyond these two items.
...
- Procedural rules to implement, trial-confirmed: disjunctive compound-objective evaluation; stale-objective invalidation (re-check the fictional state against the category's rationale at time of use, not just at declaration); S-1 winner-only Location Index eligibility.
- Additionally to implement, on a different evidentiary footing (design inference, not separately stress-tested at the table, though architecturally sound): lazy evaluation — Location Index may be derived at the point a downstream stage needs it, not mandatorily at declaration time, since Zero-Step only needs the already-recorded natural roll.
