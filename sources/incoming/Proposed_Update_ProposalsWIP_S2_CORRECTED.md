...
**Target version: v1.4.2** (update to Proposals, WIP & Design Direction v1.4.1). **Correction pass (this revision):** four fixes applied, matching the corrections made in the source v5 synthesis. No classification (Anchor status, W3 State) or open/closed status changed — only overclaiming or incomplete basis text was reworded.

1. §2.1A Conceptual Anchor table — **Armor bypass** basis reworded; "direct contradiction, not silence" overclaimed against what Proposals §5 (this same document) actually says. §5 states a Tags-based preference and never addresses location; that's a documented alternative mechanism (sufficient for Unanchored status under the Anchor test's own definition), not a stated contradiction.
2. §2.1A Conceptual Anchor table and §2.5 — **Structural weak points** given a scope note distinguishing narrative resolvability (settled) from the Index→component mapping (still open), propagated to both the Anchor table entry and the §2.5 open-questions list so the two don't drift out of sync.
3. §2.1A Procedural riders — **lazy evaluation** flagged with its correct evidence class (design inference, not equivalent to the other three trial-confirmed procedural riders).
4. §2.1A — added a lead-in caution above the Conceptual Anchor table clarifying its evidence class relative to the Named-Outcome Test (design inference vs. 21/21 trial-validated empirical finding — not equivalent evidence).
...
- Location Index evaluation may be deferred (lazy evaluation) to whenever a downstream stage first needs the answer, since Zero-Step is a read-only post-process of an already-recorded roll (Canonical §14.2). ***(Design inference — not separately stress-tested at the table, unlike the three riders above. Architecturally sound on its own reasoning, but a different evidentiary tier.)***
...
| Concept | State | Basis |
|---|---|---|
| Structural weak points (hinges, supports, load-bearing components) | **1 (Established & Resolvable)** | Resolvable through ordinary physical narration; no subsystem dependency. Note: this covers outcome differentiation only (which component was hit produces which consequence) — the numeric Location Index → component mapping itself is not codified, the same open status as anatomical mapping (§14.3) |
...
| Armor bypass | **3** | §5 below emphasises Tag/Trait-based armor interaction but does not state location cannot also participate — silence on location, not a contradiction of it. Same evidentiary footing as Disarm/Equipment Damage above |
