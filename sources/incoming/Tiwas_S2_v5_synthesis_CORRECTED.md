...
**Correction pass (this revision, supersedes the originally uploaded v5 synthesis):** Four wording/evidence-labeling corrections applied after independent review — two independent correction passes converged on the same four fixes, then merged. **No classification, ruling, or open/closed status changed** — only overclaiming or incomplete basis text was reworded. Each fix is marked inline with a `[Corrected this revision]` tag at point of change.

1. §4 / §6 — **Armor Bypass** basis reworded. "Directly contradicted"/"textual contradiction" overclaimed against Proposals §5's actual text, which favors Tags but never addresses location at all — that is silence with a stated preference, not contradiction. The "most strongly"/"most firmly" ranking is removed, since it rested entirely on the overstated contradiction claim; Armor Bypass now sits on the same evidentiary footing as Disarm and Equipment Damage.
2. §4 / §6 — **Structural Destruction (State 1)** given a clarifying note, now stated both in the table row and the Recommendation paragraph. "Resolvable through ordinary physical narration" refers to narrative differentiation only — it does not mean the numeric Location Index → structural-component mapping is itself established; that mapping is part of the still-open anatomical mapping question (Canonical §14.3).
3. §4 — added an explicit note contrasting the Anchor table's evidence class (design inference, tested against source documents) against §3's Named-Outcome Test (empirical finding, 21/21 trial-validated). These should not be read as equivalent evidence.
4. §5 — Section header and lead-in corrected. The lazy-evaluation row is explicitly tagged `[Design inference, ... not separately stress-tested]`, so the section header's former blanket claim of "Confirmed Across Multiple Rounds" overstated that one row's evidence class. Header now points to the per-row tags instead of asserting uniform confirmation.
...
|---|---|---|
| Structural destruction (hinges, supports, weak points) | **Anchored** | No subsystem dependency at all; resolvable through ordinary physical narration today. **[Corrected this revision]** This covers narrative differentiation only — the numeric Location Index → structural-component mapping remains a separate, still-open question (Canonical §14.3, Proposals §2.5), not settled by this Anchor status. |
...
| Armor bypass | **Unanchored** | Proposals §5 emphasises Armor Traits/Tags as the intended S-5 architecture but does not state that location cannot also participate (e.g., location could determine where an exposed gap is, with a Tag determining whether the gap exists or is exploitable) — silence on location, not a contradiction of it. Same evidentiary footing as Disarm and Equipment Damage below, not a stronger case against |
...
**[Recommendation]** Only Structural Destruction currently qualifies for State 1 or State 2 — and even there, only for *outcome differentiation itself* (a hinge, a support, and a lock producing different consequences via ordinary GM adjudication). The *numeric Location Index → structural component mapping* is not itself codified by anything canonical; that gap is exactly parallel to the anatomical mapping Canonical §14.3 leaves unresolved for bodies, just for structures instead. Everything else in this table sits in **State 3** until the relevant future subsystem (S-3, S-5, S-7, S-10) makes an explicit design choice that location is the delivery mechanism — at which point, and only then, it may be re-evaluated for promotion to State 2.
...
|---|---|---|
| Structural weak points (hinges, supports, load-bearing components) | **1 (Established & Resolvable)** | Only fully secure Active entry. Resolvable via ordinary GM adjudication of which component was hit — the *numeric* Location Index → component mapping is not itself codified, same open status as anatomical mapping (Canonical §14.3) |
...
| Armor bypass | **3** | §5's Traits/Tags direction is silent on whether location also participates, not opposed to it — same evidentiary footing as Disarm/Equipment Damage above, not a firmer case |
