...
**Document Version:** v1.3 (Third correction pass — supersedes v1.2 as the working Round 1 baseline)
...
**Third correction pass (this revision, v1.2 → v1.3):** Six precision corrections applied after a third independent external review. **No candidate's inclusion/exclusion status changed; no designer ruling is made here.** Each is marked inline with a `[Corrected v1.3]` tag.

1. §3.4 — **SCOPE-A evidence class reclassified.** "Architectural constraint" overstated what Proposals §3's framing actually establishes — it expresses a design emphasis, not a governing constraint on how systems must interact. Reclassified as **Design inference / candidate scope interpretation**, with wording added that Proposals §3 does not itself prohibit unopposed Core Tests from producing Effects.
2. §3.4 — **SCOPE-B × T2/T3 dependency flagged.** Quality (Canonical §13.2) is currently defined only for S-1 opposed-contest results. Combining SCOPE-B (all successful Core Tests) with a Quality-gated Eligibility/Capacity model (T2/T3) presumes an unopposed-test Quality measure that does not currently exist. Added as an explicit, unresolved dependency rather than left implicit in the scope × trigger matrix.
3. Throughout — **"Net Advantage" separated from Canonical "Quality."** Proposals §3.2's own wording is "Net Advantage/Quality bands." Quality (Margin/Blackjack/Hybrid) is a locked Canonical S-1 concept (Canonical §13.2); Net Advantage is proposal-only terminology with no Canonical definition. Wherever this document previously compressed the two into "Quality," it now preserves the distinction.
4. §3.7 — **"current Proposal default" reworded to "current Proposals/WIP draft shape."** "Default" risks implying an accepted design preference; the document's own §3.3 explicitly warns against letting the existing draft masquerade as the answer, so its comparison table should not undercut that.
5. §9 — **Summary-table wording regression fixed.** The Choose Location Interpretation B row still read "Provisionally excluded" after §5.1 was strengthened to "excluded from current S-3 design space" in the previous pass. Brought into alignment.
6. Throughout §3, §4 — **"Invariant check: Passes" reworded to "Invariant check: No conflict identified at Round-1 screening."** A candidate surviving Round 1's invariant screen has not been "validated" in any stronger sense; the original wording risked being misquoted downstream as confirmation rather than a screening result.
...
- *[Introduced v1.1]* ask the designer to lock a trigger model (T1/T2/T3) yet — see §8 below.
- *[Introduced v1.2]* **generate a Location Index for any non-attack physical resolution, under any candidate Effect.** This restates the S-2 non-attack deferral (Proposals §2.5A) as an explicit S-3 boundary, not merely an inherited silence — see §1.3.
...
| Zero-Step is deterministic; the attacker does not choose between the natural and transformed result | Canonical §14.2 | *[Introduced v1.1]* Binding on any candidate Effect that touches an existing Location Index — see §5.1, Interpretation B. |
...
| **Q2 — Effect Selection / Gating** *[Corrected v1.1]* | Given that an Effect is available, which Effect(s) from the candidate menu (Proposals §3.1) may be selected, and what — if anything — gates access to specific ones? | Whether an Effect is granted at all in the first place. |
...
### 2.1 Standardized Pipeline Terminology *[Introduced v1.1]*
...
- **Invariant check:** No conflict identified at Round-1 screening. Uses the existing Success outcome directly; introduces no new roll, no new resource, no alteration of Cost/Overflow/XP/Recovery.
...
- Additional Capacity (further Effects beyond the first) may be granted where Net Advantage/Quality bands are met — *[Corrected v1.3]* Net Advantage is Proposals-only terminology (Proposals §3.2) with no Canonical definition; Quality (Margin/Blackjack/Hybrid) is the locked Canonical S-1 concept (Canonical §13.2). This document does not treat the two as synonyms, and does not assume Net Advantage is simply an informal name for Quality.
...
- **Invariant check:** No conflict identified at Round-1 screening, for the same reasons as T1 and T2 combined; the additional-Capacity layer is strictly additive and never subtracts from or reorders the base Core Test Transaction.
...
### 3.4 SCOPE-A — Opposed-Contest-Exclusive Eligibility *[Renamed and re-axised in v1.1; evidence class corrected v1.3]*

*[Design inference / candidate scope interpretation — Corrected v1.3]* Proposals §3 frames Effects primarily around opposed contests (*"Tiwas is intended to resolve opposed contests into meaningful state change"*), but this framing does not itself establish that unopposed Core Tests are architecturally prohibited from producing Effects — it expresses a design emphasis, not a stated governing constraint on how systems must interact. The earlier "Architectural constraint" label overstated what the source text supports; that label is reserved (per Roadmap §23.2) for claims that actually govern system interaction, not for a candidate interpretation of a design-intent sentence.

Effect Eligibility applies only to results from S-1 opposed contests, never from unopposed Core Tests. Rationale as above.
...
| **SCOPE-B — All successful Core Tests** | Every success (opposed or solo) is Eligible | Any success (opposed or solo) clearing the Quality threshold is Eligible *— see dependency note below* | Every success gets base Eligibility; Quality/Tags scale Capacity/Selection *— see dependency note below* |

**SCOPE-B × T2/T3 dependency** *[Introduced v1.3]*: Quality (Margin/Blackjack/Hybrid) is currently defined by Canonical §13.2 only as a measure comparing already-rolled **successful tests within an S-1 opposed contest**. An unopposed Core Test currently has no defined Quality value under Canonical Rules v1.3. Combining SCOPE-B with T2 or T3's Quality-gated Capacity therefore presumes an unopposed-test Quality measure that does not yet exist — either a new measure would need to be defined for solo successes, or Quality-gated Effect access would need to remain restricted to S-1 results even under an otherwise-SCOPE-B eligibility rule. **This document does not resolve that question**; it is recorded here so Round 3 does not select a SCOPE-B/T2 or SCOPE-B/T3 combination without first noticing the gap.

- **Invariant check:** No conflict identified at Round-1 screening. This is a *restriction or extension of scope*, not a new mechanism.
- **Consequence:** Six combinations exist, not four alternatives — this table should replace the earlier "T1 vs T2 vs T3 vs T4" framing wherever it appears in downstream discussion. See §8, Decision Point 2. Two of the six (SCOPE-B × T2, SCOPE-B × T3) carry the open dependency noted above.
...
- **Salvage note** *[Corrected v1.2]:* A reformulation as a genuine, additional, fully-paid Core Test (own Cost/Overflow/XP/Recovery) would not violate Invariant 18, since it would *be* the Transaction rather than compete with it — recorded for completeness only, not pursued as a live candidate.
...
|---|---|---|---|---|
| T1 — Free Eligibility on any success | No conflict identified | Lowest | Lowest | Live candidate |
| T2 — Quality-threshold Eligibility | No conflict identified | Low–Medium | Medium–High | Live candidate |
| T3 — Hybrid Eligibility/Capacity/Selection (current draft) | No conflict identified | Medium | Medium–High | Live candidate; current Proposals/WIP draft shape *[Corrected v1.3]* |
| SCOPE-A/B — Eligibility-scope axis | No conflict identified (orthogonal) | N/A (modifier, combines with T1/T2/T3 per §3.4 table) | N/A | Compound axis, not a fourth alternative; SCOPE-B×T2/T3 carries an open Quality-source dependency (§3.4) |
| T5 — Persistent Effect Points | **Fail (Invariant 17)** | — | — | Excluded |
| T5-variant — Ephemeral in-test Capacity | No conflict identified, with caveat | Low (if pure read) to Medium (if spend-allocation) | Medium | Live candidate, requires separate evaluation |
| T6 — Free-standing bonus roll | **Fail (Invariant 18)** | — | — | Excluded as stated |

---

## 4. Candidate Effect Selection / Gating Models (Q2) *[Corrected v1.1]*
...
- **Invariant check:** No conflict identified at Round-1 screening.
...
- **Invariant check:** No conflict identified at Round-1 screening — Tags are already an established Reserved/Architectural category (Proposals §11) intended as consumer metadata; this is a straightforward consumer relationship, not a new primitive.
...
- **Invariant check:** No conflict identified at Round-1 screening, subject to the same §13.2 read-only constraint as Candidate T2, and the same caveat: banding Effects by Quality does not by itself establish that higher Quality *should* unlock more powerful Effects — that remains an open design question this candidate structurally enables but does not resolve.
...
- **Invariant check:** No conflict identified at Round-1 screening, as a composition of P1–P3, each individually compliant.
...
| Candidate S-3 Effect | Overlapping S-2 State-3 concept | Location-dependent candidate mechanism | Potential location-independent candidate mechanism *[Corrected v1.2]* | Fork status |
|---|---|---|---|---|
...
### 5.1 "Choose Location" — Three Interpretations, Not One Question *[Introduced v1.1]*
...
| *[Introduced v1.1]* No trigger model locked | Confirmed — §8 explicitly declines to ask for a T1/T2/T3 designer ruling this round. |
...
### Recommended Sequencing *[Introduced v1.1]*
...
Recommended ordering *[Corrected v1.2 — Round 2 is now identity-first, not weight-first]*:
...
| Option | Consequence |
|---|---|
| SCOPE-A — Opposed-contest-exclusive | Matches Proposals §3's literal framing ("resolve opposed contests into meaningful state change"); keeps unopposed Core Tests (e.g., a solo Extended Test interval) mechanically simpler; no Quality-source dependency since Quality is already S-1-native |
| SCOPE-B — All successful Core Tests | Broader, more uniform application of Effects across the system; raises the question of what an "Effect" even means for a non-contest success (e.g., a solo lockpicking success) — not explored in this round; **if paired with T2/T3, also inherits the open Quality-source dependency flagged in §3.4** (Quality is currently defined only for S-1 opposed-contest results) |
...
| "Choose Location" Interpretation B (choose the Index result) | **Excluded from current S-3 design space** *[Corrected v1.3]* — conflicts with locked Canonical §14.2; reopening requires an explicit Canonical §14.2 reopening, not an S-3-level decision |
