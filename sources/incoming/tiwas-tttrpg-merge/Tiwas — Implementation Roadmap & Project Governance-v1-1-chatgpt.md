...
| 2 | S-2 | Hit-location policy | **WIP** | S-1 |
...
Implement the architecture before finalising the universal policy.
...
- Tier 1;
- Tier 2 interface;
- scene/campaign configuration;
- Location Index interface.

Acceptance tests:

- Tier 0 creates no location state.
- Tier 1 requires no mandatory additional die.
- Tier selection never alters the original roll.
- Location data is post-resolution.
- Location data can be consumed by later Effects/Wound/Armor systems.
...
Do not lock the universal default before this evaluation.
