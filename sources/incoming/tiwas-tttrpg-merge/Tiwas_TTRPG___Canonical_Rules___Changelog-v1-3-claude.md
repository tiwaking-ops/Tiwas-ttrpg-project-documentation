...
## 14.5 E9 human playtest record

The E9 usability playtest used two physical d10s and 50 participants in ten groups of five: 25 participants new to RPGs and 25 RPG players new to Tiwas. Participants reported no difficulty rolling the dice or exchanging the digits for hit location, and feedback was positive.

This is empirical usability evidence. It supports the designer ruling but does not resolve the remaining S-2 architecture.

## 14.6 Comparative validation against Units-Digit

Following the E9 usability finding above, the Tier-1 provider choice was additionally validated against the candidate alternative (Units-Digit, a single-digit derivation) through exhaustive comparative analysis:

- Zero-Step's 100-value domain provides 1% native resolution; Units-Digit's 10-value domain provides 10%.
- Against representative test tables, Zero-Step's maximum distribution error was exactly one-tenth of Units-Digit's, and Units-Digit rendered a low-weight zone structurally unreachable under all possible orderings of one such table — proven algebraically, not merely observed across samples.
- Both derivations can inherit order-dependent error from a downstream index-to-zone mapping stage; when this occurs, Units-Digit's resulting error is exactly ten times larger than Zero-Step's, tracking the domain-size ratio.
- A tested symmetry-preserving repair for Units-Digit restored balance between paired zones but increased overall distribution error elsewhere. Logged as a rejected experimental repair, not proof that no repair could work.

This validation is retained as historical design evidence supporting the ruling in §14.1, consistent with how S-1's validation evidence is treated (§13.6) — not as a mathematical rule in its own right.

A further comparative usability check (derivation cost of Zero-Step versus Units-Digit across rolling methods other than two physical d10s) was scoped but not conducted. The ruling proceeds without it, as an accepted, disclosed, non-blocking residual.
...
1. Added comparative validation of Zero-Step against Units-Digit (§14.6), following exhaustive analysis conducted after the v1.2 lock.
2. Recorded the comparative-derivation-cost residual (rolling methods other than two physical d10s) as accepted and disclosed, not blocking.
3. No change to the ruling itself (§14.1) or to any other locked mechanic.
