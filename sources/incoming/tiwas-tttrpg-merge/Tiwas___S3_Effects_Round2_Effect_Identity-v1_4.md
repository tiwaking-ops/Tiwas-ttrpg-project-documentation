...
**Document Version:** v1.4 (Micro-cleanup pass — supersedes v1.3 as the working Round 2 baseline)
...
**Cleanup pass (this revision, v1.1 → v1.2):** Three textual corrections applied after a second independent external review. **No identity conclusion, dependency finding, or open-question status changed — all three are wording-consistency fixes.** Marked inline with `[Corrected v1.2]`.

1. §12 — **Required.** The Effect 8 summary row still read "may not fit the Track A/Track B interface," which contradicted the v1.1 correction made in §9 (that removed exactly this implication from the per-Effect analysis). The stale phrase was never propagated to the consolidated table. Reworded to match §9: the state transition has no identified universal state interface, which is an open architectural question, not evidence against the candidate.
2. §9 — Recommended. Effect 8's "the mechanical hook is the least defined of the ten" reworded to "among the least mechanically specified of the ten," to avoid an unintended ranking conflict with §6's description of Seize Tempo as "the candidate Effect with the least-resolvable identity in this round" — these are different concepts (least mechanically specified vs. blocked on an undefined prerequisite subsystem) and should not read as competing superlatives.
3. §11 — Recommended. "Interpretation C is the only one of the three with a fully specified mechanical role" reworded to "clearly mechanical role," since the same sentence immediately notes Interpretation C depends on the unresolved S-2 anatomical mapping — "fully specified" overstated that.

Reviewer point not adopted as a correction (recorded, not applied): a caution that "location-flavored" (Effect 1, §2) should not become shorthand for the S-2 investigation's specific term "location-anchored" as Round 2b/3 proceeds. This is a forward-looking governance note for future rounds, not a defect in v1.1's wording, and is carried forward as guidance below rather than as a textual change here.

**Attribution correction (this revision, v1.2 → v1.3):** One precision edit applied after a third independent external review. **No identity conclusion, dependency finding, or open-question status changed.** Marked inline with `[Corrected v1.3]`.

1. §11 (Effect 10 — Choose Location, Interpretation B) — "Fails — Canonical §14.2, locked" overstated the attribution: Canonical §14.2 establishes Zero-Step's determinism, but Canonical §14.3 separately leaves open whether a later rule may select, modify, or consume a Location Index at all. The actual source excluding Interpretation B from current S-3 design space is the accepted non-canonical S-2 candidate policy (Round 1 §5.1, §9), not a direct textual prohibition in §14.2 itself. Reworded to attribute the exclusion correctly — to the architectural constraint (§14.2's determinism) plus the S-2 candidate policy's treatment of it — without implying Canonical §14.2 explicitly contains a "Choose Location B is prohibited" rule, and without suggesting §14.3's broader open question has been resolved.

Reviewer point not adopted as a correction (recorded, not applied): a note that Effect 8's "open architectural question" framing (lacking a universal state interface) should not be upgraded into an architectural requirement. The current v1.2/v1.3 wording already keeps this as a design inference/open question, consistent with the reviewer's own recommendation to leave it as-is — no text changed.

**Micro-cleanup pass (this revision, v1.3 → v1.4):** Four minor precision edits applied after a fourth independent external review, none altering a conclusion, dependency, or status. Marked inline with `[Corrected v1.4]`.

1. §7 (Effect 5 — Seize Tempo) and §12 (cross-Effect observations) — "the candidate Effect with the least-resolvable identity in this round" / "have the least resolvable identities in this round" reworded to "among the least-resolvable identities in this round," since Effect 8 and Choose Location Interpretation A also carry unusually weak mechanical identity — the same competing-superlative risk the v1.1 Effect 8 correction already addressed once, recurring here in different wording.
2. §14 (Summary Table) — "All ten Effects' identity-checklist answers — Recorded" reworded to "Identity-checklist analysis for all ten Effects — recorded, with unresolved dependencies and forks explicitly identified," so the entry doesn't read as implying all eleven questions were answered conclusively for every Effect.
3. §9 (Effect 8 — Open Retreat / Compel Yield) — "the only Effect whose primary consequence is social/psychological" reworded to "the clearest candidate whose primary consequence is social/psychological," since Conditions (Effect 2) are deliberately heterogeneous and could in principle include a social/psychological sub-case — "only" claimed more exclusivity than the analysis supports.
4. §2 (Effect 1 — Inflict Injury) — "Marks the target as having sustained a meaningful injury" reworded to "Represents the target as having sustained a meaningful injury," keeping the S-4 interface explicitly provisional rather than reading as a definite mechanical write.
...
| 2. Mechanical action | **[Corrected v1.4]** Represents the target as having sustained a *meaningful* injury beyond ordinary Overflow-to-HP damage, for eventual S-4 Wound Engine consumption. |
...
| **Invariant status** | No conflict identified | **[Corrected v1.3]** Fails the current S-2 architectural constraint: Zero-Step is deterministic and the natural roll remains authoritative (Canonical §14.2). Canonical §14.3 separately leaves open whether a later rule may select, modify, or consume a Location Index at all — that broader question is not what excludes Interpretation B. The operative exclusion is the accepted non-canonical S-2 candidate policy, which treats choosing the Index result as outside the current design space (Round 1 §5.1, §9). Excluded from current S-3 design space; not carried further in this document; not itself a new Canonical ruling. | No conflict identified, with the dependency caveat below |

**11. Differentiation (Effect-level, not per-interpretation):** "Choose Location" is unique among the ten candidates in that its identity question is not "what does it mechanically do" in isolation, but "which of three non-equivalent things is it." Interpretation A is closest in character to a no-op flavor declaration (weakest mechanical identity of the ten). Interpretation C is the only one of the three with a **[Corrected v1.2]** clearly mechanical role, but that role is explicitly a **dependency on S-2's still-open anatomical mapping** (Proposals §2.4–§2.5), not something S-3 can complete unilaterally — per Round 1 §5.1's "dependency, not identity" note, developing Interpretation C would consume that mapping as a named prerequisite, not discharge S-2's obligation to define it. This document does not select among A/B/C; per Round 1 Decision Point 3, that remains open.
...
| Blocked on a wholly undefined prerequisite subsystem | Effect 5 (Seize Tempo) — blocked on Proposals §12 (Time/Action Economy, no draft direction at all); Effect 6 (Guard Break) — blocked on Proposals §6 (S-6 Defense, Reserved/Proposed with only a rejected Passive Guard candidate on record) | **[Corrected v1.4]** These two candidates are among the least-resolvable identities in this round, for different reasons than the location-dependence forks affecting Effects 3, 9, and (partially) 2 and 1, and distinct from Effect 8's and Choose Location Interpretation A's weak-identity character (§9, §11) |
...
| Sole narrative/morale-primary Effect | Effect 8 (Open Retreat/Compel Yield) | **[Corrected v1.2]** Its mechanical hook is among the least mechanically specified of the ten, and its participation-state transition does not yet have an identified universal state interface. This is an open architectural question, not a rejection of the candidate. Flagged for Round 2b, not resolved |
...
|---|---|
| **[Corrected v1.4]** Identity-checklist analysis for all ten Effects | Recorded (§2–§11), with unresolved dependencies and forks explicitly identified rather than answered conclusively; several answers depend on other open forks/subsystems rather than being fully self-contained |
...
| v1.2 cleanup pass | Applied — required fix to a stale §12 phrase left inconsistent by the v1.1 Effect 8 correction, plus two recommended precision edits (Effect 8 "least defined" ranking language; Interpretation C "fully specified" overstatement). No identity conclusion or dependency finding changed. |
| v1.3 attribution correction | Applied — Interpretation B's exclusion re-attributed from a direct "Canonical §14.2, locked" claim to the correct combination of §14.2's determinism constraint and the S-2 candidate policy's treatment of it, without overstating that Canonical §14.3's broader open question has been resolved. No identity conclusion or dependency finding changed. |
| v1.4 micro-cleanup pass | Applied — four precision edits (Seize Tempo/§12 "least-resolvable" ranking language softened to "among the least-resolvable"; §14 summary-row wording; Effect 8 "only Effect" → "clearest candidate"; Effect 1 "marks" → "represents" to keep the S-4 interface explicitly provisional). No identity conclusion or dependency finding changed. |
