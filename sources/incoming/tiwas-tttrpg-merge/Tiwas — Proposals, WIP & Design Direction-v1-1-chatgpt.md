...
# 2. S-2 — Hit-Location Default Policy

**Status: WIP / Draft / Not Locked**

The current S-2 draft explicitly states that nothing in it is settled.
...
## 2.2 Location-granularity tiers
...
| 1 | Lightweight derivation from the existing attack roll | Near-zero | Standard simulation-grade play, formal duels |
| 2 | Full location / armour / surgery model | Higher | Optional gritty play |

**Important:** These location Tiers are unrelated to Skill Tier.

Core Skill Tier measures the number of attributes in a Skill formula.

Location Tier measures physical-location granularity.

They share terminology only.

## 2.3 Architectural principle

Hit location is intended as a read-only post-process.

It consumes the historical roll produced by the Core Test Transaction or an S-1 participant roll.
...
No location information is generated.

No location state exists.

## 2.5 Tier 1

The current draft investigates deriving a Location Index from the attacker's own natural roll without requiring an additional die.

The Location Index is a numeric intermediate value.

It is not itself a body-part name.

A separate anatomical mapping table is required to convert an index into an anatomical zone.

No canonical anatomical table currently exists.

## 2.6 Tier 2

Tier 2 is recognised as a future high-granularity module.
...
They must not be invented merely to make this document appear complete.

## 2.7 Current design question

The principal S-2 decision is:

> Should the default location tier be universally fixed, or should location granularity be selected by situation/scene?

Current design direction strongly favors **situation-dependent selection**, but this remains a design direction rather than a locked mechanical rule.
...
|---|---|---|
| S-2 | Hit-location tier/default policy | WIP |
