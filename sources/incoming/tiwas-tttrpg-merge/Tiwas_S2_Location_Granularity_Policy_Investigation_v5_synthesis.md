...
**Status: Consolidated recommendation. NON-CANONICAL. Requires explicit designer ruling before any Proposals/WIP or Roadmap update, and cannot modify Canonical Rules under any circumstance.**
...
| **Explicit-only objective boundary** — the GM does not infer an unstated distinct objective from location or fictional context alone; only stated objectives are Warrant-eligible | Tested cleanly (21/21, Round 3, including the #16/#17 minimal pair) — **but see §8, this remains an unconfirmed designer decision, not an adopted rule** |
...
## 8. Explicit Unresolved Designer Decisions

| # | Decision | Status |
|---|---|---|
| 1 | **Explicit-only vs. inferred objectives.** Tested cleanly at 21/21 for explicit-only; never affirmatively ruled by you. This synthesis assumes explicit-only as the *tested default*, not an adopted rule. | **Open — requires your ruling** |
| 2 | Whether States 1–4 should be GM-facing as four distinct labels, or collapsed into the single operational question confirmed by the second blind test (`Generate = Warrant ∧ Resolvable`, with Conceptual Anchor folded silently into how Warrant is taught rather than exposed as a fourth term a GM must reason about live) | Open — this synthesis presents the full model for design-documentation purposes; a simplified GM-facing procedure derived from it is still a separate drafting task |
| 3 | Whose roll supplies a Location Index for non-attack physical resolutions (§7) | Open, out of scope for this synthesis |
| 4 | Whether/when States-3 entries (Disarm, Equipment, Impairment, Armor Bypass, Incapacitation) should be revisited — tied to S-3/S-5/S-7/S-10 locking a location-specific mechanism, not merely locking numbers | Open, dependent on future subsystem work |
| 5 | This entire policy rests on **single-designer, single-evaluator table-practice evidence** (two trials, 35 total scenario-responses) plus document-grounded conceptual analysis. No multi-GM usability validation has occurred. | Open — optional validation, not required to understand what the rule means, per the prior round's own framing |
