---
document:
  title: "Tiwas — Implementation Roadmap & Project Governance"
  version: "v1.4.3"
  status: "Non-canonical (self-declared: \"Rule Authority: None\") — implementation/process guidance"
provenance:
...
consolidation_note: >
  Placed in roadmap/ during initial consolidation (2026-08-29). Content below
  the header is UNMODIFIED from sources/tiwas-implementation-roadmap-and-project-governance-v1.4.3.md.
  This document explicitly disclaims game-mechanic authority; placement outside
  canonical/ reflects that self-declaration, not an independent re-decision.
---
...
| 4 | S-4 | Wound activation/severity | **Partially Ruled (DEC-032–DEC-037): terminology lock (Injury=HP, Wound=numerical localized state); trigger = selectable Effect only (Wound realized as "Wounded" Condition via Impose Condition); Overflow never causes Wounds; Track A/B both apply sequentially; severity = numerical count, Light/Serious/Critical descriptive only; DEC-020 reopened and LI mechanism defined (DEC-037). Remaining open: numerical thresholds (OPEN-006), consequences (OPEN-007).** | S-2 and S-3 |
