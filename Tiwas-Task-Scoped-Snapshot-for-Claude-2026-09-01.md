...
**Open.** All S-5…S-12 are Open/Reserved/Proposed — no locked mechanic for any of them. **Current state of the umbrella:**
- **S-5** is ruled (DEC-058…DEC-062) — all five forks resolved.
- **S-6** is ruled (DEC-044…DEC-050) — Active Defense, all seven forks resolved.
...
- **S-12** (Creature/Campaign) — open, content dependency on S-4/S-41 anatomical mapping. **Remaining open: S-12 only.**
...
| **S-12** | Creature/Campaign content | — | **Open** | Creature coverage individual templates (DEC-041 §5); content dependency. |
...
**Upstream dependencies:** DEC-016 (Architectural Invariants, esp. #6/Invariants 6/10), DEC-010 (Skill Roll Pool), DEC-006/007/009 (Core Test/Cost/Failure XP). **Key design constraint:** Difficulty modifiers act on the Skill side only — never on the natural die roll (Invariant 6 preserved). B1+C2 together require **no amendment to any Core Invariant** (Invariant 10 preserved exactly as currently worded).
...
## 3.4 OPEN-009 — S-6 positive-Effect Active Defense mitigation
...
- Upstream: DEC-050 (universal eligibility), DEC-048 (Model B), DEC-049 (separate mitigation per Effect), DEC-027 (auto-apply).

## 3.5 OPEN-010 — S-6 repeated-Defense fatigue/exhaustion
...
## 3.6 Confirmed-closed items (so you do not re-open them silently)
...
| S-3 Effect identity & multi-effect opposition investigation | Closed (all items ruled) | `investigations/tiwas-s3-effect-identity-and-multi-effect-opposition-investigation-v0.1-open.md` |
...
*End of snapshot. Dated 2026-09-01, v3.0. Task-scoped; not authoritative beyond this session.*
