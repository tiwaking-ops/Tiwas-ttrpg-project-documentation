# Tiwas-TTRPG — Advisory Session Brief (2026-08-31)

**Type:** Dated, task-scoped SNAPSHOT for a cold-reading advisory model. **Not** the authoritative corpus.
**Compiled by:** OpenCode (documentarian), with live repository access, 2026-08-31.
**For:** The advisory design-assistant model (Claude) loading this file cold at the start of a session. No memory of
prior sessions. No repository access while this file is loaded.
**Usage:** Tiwa uploads this file; the advisory model reads it cold and gives **advisory opinions only** on the Open
decisions below. Tiwa verifies anything it acts on against the real repository, separately, with OpenCode.
**Purpose:** Enough context that the advisory model can state, for every **currently-Open** decision in the live
decision register, (a) what is ruled/locked upstream, (b) what forks remain open, and (c) what governance rules
constrain any future ruling — plus the repository's actual formal-reporting/provenance standard so any document it
produces during the session conforms to that standard on the first attempt.
...
designer (Tiwa) for this single task (2026-08-31)** so a cold-reading model can receive one self-contained, curated
brief. This is a **one-session exception**: it does not repeal the standing prohibition generally, does not reinstate
any prior consolidated merge file (e.g. `TTTRPG-merge-v2.md` or `sources/SOURCE_CORPUS_ORIGINAL_TTTRPG-merge-v1.md`)
as a standing artifact, and generating this file establishes no precedent that another merged file may be created.
This file is disposable working material; it may be deleted after use and is not a repository artifact.

**Pointers, not inline evidence.**
Items hash-indexed as evidence (per-decision handoff reports, raw LLM survey transcripts, full investigation
threads) are **pointer-referenced** (path + one-line status) and **not reproduced inline**, per the standing
...
non-canonical designer ruling (e.g. DEC-023–DEC-050) — it governs candidate material only and is NOT canonical.**
...
depend on. "Authority" is as the register records it, not an external assertion. **The register (`_consolidation/
decision-register.md`) is the live source of truth for current status**; the roadmap may lag it.
...
| DEC-023–DEC-031 | S-3 Outcome Effects | **DEC-023** tiered Effect menu (base tier = Inflict Injury HP-only + Open Retreat/Compel Yield; five gated tiers Position → Time/Action, Condition → Conditions, Equipment → Equipment, Defense → S-6, Location → S-2 invocation promotion); **DEC-024** flat one-Effect-per-win (second Effect needs separate opposed roll); **DEC-025** pure declared intent (no Skill-side gating); **DEC-026** second-Effect roll = different Advanced Skill, **defensive roll deferred to S-6**; **DEC-027** auto-apply default (contested application deferred to S-6 as optional modifier); **DEC-028** combined Location+Tag gating (narrowed by DEC-041); **DEC-029** boundary; **DEC-030** fail-and-fall-back; **DEC-031** Quality gates eligible Effects | Ruled (non-canonical) |
...
| DEC-037 | S-2/S-4 Non-Attack Location Index Generation | (1) Failed governing Core Test supplies digits for Zero-Step; (2) hazard "win"; (3) systemic exempt (drowning, suffocation, extreme temp, poison); (4) passive numeric-stub fallback. Closes OPEN-008. Residual flags resolved by DEC-038/039 | Ruled |
...
| DEC-039 | OPEN-001B — Extended Test governing roll | **Final roll** of an Extended Test is governing for Location Index; no earlier roll stored/referenced | Ruled |
...
| DEC-041 | OPEN-003 — Anatomical mapping | Skill-Tier-gated granularity, **universal gate** (six-part rule): (1) Location Index only when roll promoted to Location-Tier 1/2+ **AND** acting skill is Skill-Tier 2+ (Advanced; base = Tier-1); (2) Tier-1 → coarse zones via anatomically-weighted ranges; (3) left/right = Zero-Step digit parity odd/left, even/right; (4) Tier-2 → granularity scales with Skill-Tier; (5) creature templates per type; (6) gate universal across all location-referencing Effects. **Narrows DEC-033 & DEC-028** | Ruled |
...
| DEC-044 | OPEN-005 S-6 Fork 3 — Defense architecture | **RULED — Active Defense**: defender makes a genuine Core Test vs incoming Effect (full PE/MP cost, full Core consequences DEC-006/007). Passive/contest-participant/resource-costed no longer live | Ruled |
| DEC-045 | OPEN-005 S-6 Fork 4 — Who rolls | **RULED — the defender rolls** | Ruled |
| DEC-046 | OPEN-005 S-6 Fork 5 — Voluntary decline | **RULED — Yes**, defender may decline and accept the Effect as applied | Ruled |
| DEC-047 | OPEN-005 S-6 Fork 7 — Ceiling | **RULED — Uncapped**, no per-action limit. Follow-up (deferred): fatigue/exhaustion from repeated Defense rolls — no such system exists; unscoped. → OPEN-010 | Ruled |
| DEC-048 | OPEN-005 S-6 Fork 2 — Timing vs DEC-027 | **RULED — Model B**: Effect auto-applies per DEC-027 literally; Active Defense = post-hoc mitigation, not a gate. DEC-027 not contradicted | Ruled |
| DEC-049 | OPEN-005 S-6 Fork 6 — DEC-026 deferred position | **RULED — separate mitigation per Effect**: each Effect gets its own independent Active Defense roll (two Effects → two rolls) | Ruled |
| DEC-050 | OPEN-005 S-6 Fork 1 — Defensible-Effect scope | **RULED — universal eligibility**, no enumerated/gated list. Reasons: player agency; S-3 Effect menu undefined; **menu includes positive Effects**. Follow-up flagged (open) → OPEN-009 | Ruled |
...
## 3. Currently-Open decisions (from `_consolidation/decision-register.md` §C, live post-DEC-050)

**The currently-Open register items are OPEN-005, OPEN-007, OPEN-009, and OPEN-010.** OPEN-001/002/003/004 are closed
(DEC-038/040/041/042); OPEN-006 resolved via DEC-035; OPEN-008 closed via DEC-037. OPEN-007 is listed in §C but is
recorded as **Ruled** already — retained for completeness, not open. The genuinely-outstanding items are the OPEN-005
subsystem question plus the two follow-ups OPEN-009 and OPEN-010.
...
| **What the corpus says** | All S-5…S-12 subsystems are "Open"/"Reserved"/"Proposed" — **no locked mechanic for any of them**. S-4 is now ruled (DEC-032–DEC-037). S-6's seven forks are **ruled at brief level** (DEC-044–050) but Promotion Rule steps 5–8 are outstanding. |
| **Upstream ruled/locked** | DEC-001–DEC-016 (Core, incl. DEC-013 S-1, DEC-014 Zero-Step/Tier-1, DEC-016 invariants). S-3 Effects ruled (DEC-023–031). S-4 wound chain ruled (DEC-032–037). Non-attack provenance chain resolved (DEC-020→DEC-036→DEC-037; DEC-038/039). S-6 Defense forks ruled (DEC-044–050). S-8 third-party adjudication ruled (DEC-043). |
| **Forks remaining** | **Sequencing:** which subsystem to design next. **Per-subsystem space** for **Armor (S-5)**, **Defense (S-6 — forks ruled; formal/Canonical steps and the OPEN-009/010 follow-ups remain)**, **Incapacitation/death (S-7)**, **Difficulty/task adjudication & Stakes Gate (S-8)**, **Extended Test progress (S-9)**, **Extended Test failure loss (S-10)**, **Rest/Healing (S-11)**, **NPC compression (S-12)**. Roadmap §6.1/§6.2 dependency layering: S-5/S-6 → S-7/S-8 → S-9/S-10 → S-11/S-12; S-5 depends on S-3+S-4, S-6 on S-1, S-7 on S-4, S-8 on Core, S-9 on S-1, S-10 on S-9, S-11 on S-4+S-7, S-12 on preceding systems. |
| **Governance constraints** | Nothing here may be resolved by an advisory model; only the human designer rules, and promotion requires the 8-step Promotion Rule (§1.4). Any subsystem producing a Location Index must honor DEC-014's scope limit and Invariants 11/18. S-8's Stakes Gate is flagged as the **most likely natural trigger** for reopening the S-2 non-attack deferral (Roadmap §11.2/§13). S-6 carries DEC-026 & DEC-027 waiting on its resolution (now at brief level). |
...
| **What the corpus says** | **RULED already — included only for completeness; do not treat as open.** Wounds carry individual mechanical attribute penalties; different wounds may have different magnitudes (−1/−5/−30/−80/−100); more/severe wounds → more negatives; accumulating too many → "game overed"; weaker characters reach that point faster; healing cost scales with the magnitude of the negative; Greater Wound Effects cause serious negatives. |
| **Upstream ruled/locked** | DEC-032 (Injury vs. Wound), DEC-033 (trigger & Location scope, narrowed by DEC-041), DEC-034 (Track A/B), DEC-035 (severity from Effect). |
| **Forks remaining** | None currently open — recorded as Ruled. |
| **Governance constraints** | Recorded as a designer ruling; promotion still requires the 8-step Promotion Rule. |

### Open-3 — OPEN-009: S-6 positive-Effect Active Defense mitigation (NEW)

| | |
|---|---|
| **Register row** | Decision-register §C OPEN-009 |
| **What the corpus says** | Raised by DEC-050's universal-eligibility ruling (Fork 1): since any auto-applied Effect is eligible for Active Defense mitigation **and the S-3 Effect menu includes positive Effects** (e.g. an opponent's successful advantageous Effect), whether "mitigating" a positive Effect is **coherent** — and whether it's **desired** — is unresolved. |
| **Upstream ruled/locked** | DEC-050 (universal eligibility), DEC-044/048 (Active Defense, Model B post-hoc mitigation), DEC-027 (auto-apply), DEC-023 (S-3 Effect menu structure — menu is currently **undefined** beyond DEC-023's tier skeleton). |
| **Forks remaining** | The core question is open: does Active Defense mitigation sensibly apply to positive Effects, and should it? Cannot be scoped to a full design until the S-3 Effect menu is actually defined (currently only the DEC-023 tier skeleton exists). Not blocking current S-6 work. |
| **Governance constraints** | Advisory model may present interpretations/recommendations but may not rule. Resolution is a designer ruling + (for canon) the 8-step Promotion Rule. Requires the S-3 Effect menu to be defined first. |

### Open-4 — OPEN-010: S-6 repeated-Defense fatigue/exhaustion (NEW)

| | |
|---|---|
| **Register row** | Decision-register §C OPEN-010 |
| **What the corpus says** | Follow-up from DEC-047 (uncapped Defense rolls): whether repeated Defense rolls within a scene/encounter should trigger additional negative effects from fatigue/exhaustion. |
| **Upstream ruled/locked** | DEC-047 (uncapped Defense rolls — no per-action ceiling). |
| **Forks remaining** | The whole concept is unscoped: **no fatigue/exhaustion system currently exists anywhere in the game**, so nothing can be scoped as a rule until/unless such a system is designed. Non-blocking. |
| **Governance constraints** | Advisory model may discuss the *principles* of attaching fatigue to repeated rolls, but any such system would itself be a new design that requires designer approval (cf. Invariants 17/18 — no new primary resource/currency without approval, if it touches resources) plus the Promotion Rule. No existing system to hang it on. |

### Closed-for-context (struck in register §C — load-bearing upstream, not open)

| ID | Closed via | One-line | Remains relevant to |
|---|---|---|---|
| OPEN-001 | DEC-038 + DEC-039 | H0 Rider B sub-options → single causal-attribution rule; Extended-Test final-roll-govems | S-2 non-attack provenance, S-9/S-10 Extended Tests |
...
**not** paraphrased or reconstructed. Any document produced during the session (draft ruling reports, handoff
...
> `**Prepared By:** Claude Sonnet 5 (claude-sonnet-5), in the Lead Systems Architect / Design Assistant role for
...
Each ruling drafted in-session should map cleanly onto the register's row structure:
...
and register-column alignment). The current register row for the exemplar's subject (DEC-033, DEC-034, DEC-037) shows
the register-column alignment in practice.
...
record, raw LLM survey transcripts, the S-6 brief's advisory input) is referenced by **path + one-line status**,
never reproduced inline. Example:
...
- `investigations/tiwas-s6-defense-opening-brief-2026-08-31.md` — S-6 Defense opening brief; records the seven fork
  rulings (§1a) and the follow-up flags behind OPEN-009/010 (pointer for context, not reproduced in full).
...
- The two distinct "Tier" concepts must not be conflated: **Skill-Tier** (1+, attribute count; base = 1, Advanced =
  ...
  misled a draft; keep them separate (see DEC-041).
- S-6's seven forks are **ruled** (DEC-044–050) but remain non-canonical and un-promoted; when advising on OPEN-005,
  distinguish "fork resolved at brief level" from "promoted to Canonical" (the latter has not happened).
